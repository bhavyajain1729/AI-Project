/* PSC: xxdoaemail.p - DOA Email Notification							*/
/*                                                                      */
...
IF ipc_businessLine NE "" AND
   NOT CAN-FIND(FIRST code_mstr NO-LOCK
				WHERE code_domain 	EQ global_domain 		AND
					  code_fldname 	EQ "DOA_BUSINESS_LINE" 	AND
...
/*
qh:QUERY-PREPARE("FOR EACH " + lvc_tablename + " NO-LOCK WHERE RECID(" + lvc_tablename + ") = " + STRING(ipr_recid)).
qh:QUERY-OPEN.
qh:GET-FIRST().

IF htable:AVAILABLE
THEN
	ASSIGN lvc_rulecode = htable:BUFFER-FIELD(ENTRY(1,lvc_tablename,"_") + "_RuleCode"):BUFFER-VALUE.
	...
	FOR EACH xxdoarule_mstr
		...
		THEN DO:
			...
			THEN DO:
				...
				/*
				IF htable:BUFFER-FIELD(ENTRY(1,lvc_tablename,"_") + "_CurrentAppr"):BUFFER-VALUE EQ ""
				THEN DO:
				*/
				DO lvi_count = 1 TO 10:
					IF xxdoarule_approvers[lvi_count] NE ""
				...
				ELSE DO:
					DO lvi_count = 1 TO 10:
						IF xxdoarule_approvers[lvi_count] NE ""
						THEN DO:
							IF htable:BUFFER-FIELD(ENTRY(1,lvc_tablename,"_") + "_CurrentAppr"):BUFFER-VALUE EQ xxdoarule_approvers[lvi_count]
							THEN 
								LEAVE.
						END. /* IF xxdoarule_approvers[lvi_count] NE "" */
					END. /* DO lvi_count = 1 TO 10 */
					...
					DO lvi_count = (lvi_count + 1) TO 10:
						IF xxdoarule_approvers[lvi_count] NE ""
						THEN DO:
							ASSIGN lvc_nextapprover = xxdoarule_approvers[lvi_count].
							LEAVE.
						END. /* IF xxdoarule_approvers[lvi_count] NE "" */
					END. /* DO lvi_count = 1 TO 10 */
				END.
...
ASSIGN lvc_nextapprover = REPLACE(lvc_nextapprover,"SELF",global_userid).
...
RUN getGCMData("xx_DOA_CCO", "CCO", INPUT-OUTPUT lvc_nextapprover, INPUT-OUTPUT lvc_notifyto).
RUN getGCMData("xx_DOA_CEO", "CEO", INPUT-OUTPUT lvc_nextapprover, INPUT-OUTPUT lvc_notifyto).

IF LOOKUP(lvc_nextapprover,lvc_doaapprovercodeList,",") GT 0
THEN DO:
	/* APPROVER DETAILS NOT DEFINED */
	ASSIGN opc_errorMsg = lvc_nextapprover + " approver not defined for Rule Type - " + ipc_ruletype + ", Business Line - " + ipc_businessLine + ", Site - " + ipc_site.
	RETURN.
END.
...
/*
DO TRANSACTION:
	qh:QUERY-PREPARE("FOR EACH " + lvc_tablename + " EXCLUSIVE-LOCK WHERE RECID(" + lvc_tablename + ") = " + STRING(ipr_recid)).
	qh:QUERY-OPEN.
	qh:GET-FIRST().

	IF htable:AVAILABLE
	THEN DO:
		htable:BUFFER-FIELD(ENTRY(1,lvc_tablename,"_") + "_RuleCode"):BUFFER-VALUE = lvc_rulecode.
		
		IF opl_isfinalapproved 
		THEN DO:
			htable:BUFFER-FIELD(ENTRY(1,lvc_tablename,"_") + "_CurrentAppr"):BUFFER-VALUE = "".
		END.
		ELSE DO:
			htable:BUFFER-FIELD(ENTRY(1,lvc_tablename,"_") + "_CurrentAppr"):BUFFER-VALUE = lvc_nextapprover.
			htable:BUFFER-FIELD(ENTRY(1,lvc_tablename,"_") + "_ApprovalStatus"):BUFFER-VALUE = "Pending".
		END.
...
PROCEDURE getGCMData:
	DEFINE INPUT PARAMETER 			ipc_codefldname 	AS CHARACTER NO-UNDO.
	DEFINE INPUT PARAMETER 			ipc_ApproverCode  	AS CHARACTER NO-UNDO.
	...
	DEFINE VARIABLE 				lvc_codevalue 		AS CHARACTER NO-UNDO.
	DEFINE BUFFER 					buff_code_mstr 		FOR code_mstr.
	
	FOR EACH buff_code_mstr
		WHERE buff_code_mstr.code_domain 	EQ global_domain 	AND
			  buff_code_mstr.code_fldname 	EQ ipc_codefldname 	AND
			  buff_code_mstr.code_value   	NE ""
		NO-LOCK:
		IF lvc_codevalue = "" 
		THEN
			ASSIGN lvc_codevalue = buff_code_mstr.code_value.
		ELSE 
			ASSIGN lvc_codevalue = lvc_codevalue + "," + buff_code_mstr.code_value.
	END. /* FOR EACH buff_code_mstr */
	
	IF lvc_codevalue NE ""
	THEN
		ASSIGN
			iopc_nextapprover = REPLACE(iopc_nextapprover,ipc_ApproverCode,lvc_codevalue)
			iopc_notifyto     = REPLACE(iopc_notifyto,ipc_ApproverCode,lvc_codevalue).
END. /* getGCMData */
